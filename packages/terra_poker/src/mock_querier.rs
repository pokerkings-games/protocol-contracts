...
use cosmwasm_std::{Binary, CanonicalAddr, Coin, ContractResult, Decimal, from_slice, OwnedDeps, Querier, QuerierResult, QueryRequest, SystemError, SystemResult, to_binary, Uint128, WasmQuery, from_binary, BankQuery, AllBalanceResponse, Addr, CustomQuery, Api};
...
impl WasmMockQuerier {
    ...
    fn handle_wasm_smart(&self, contract_addr: &String, msg: &Binary) -> QuerierResult {
        let mut result = self.handle_wasm_smart_governance(contract_addr, msg);
        ...
        match from_binary(msg) {
            Ok(GovQueryMsg::ContractConfig {}) => {
                let response = GovContractConfigResponse {
                    staking_token: self.governance_querier.staking_token_contract.clone(),
